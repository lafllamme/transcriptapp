...
def main():
	...
	if (choice == menu[2]):
		...
		if col2.checkbox('🔴 Transcribe Live'):
			try:
				audioLength = st.slider(
				"Anzahl der Aufnahme in Sekunden", min_value=1, max_value=20, value=10, step=1)
				if col2.checkbox('🎤 Record Mic'):
					with st.spinner('Recognizing...'):
						...
						with sr.Microphone() as source:
							...
							audio_data = r.record(source, duration=audioLength)
							# convert speech to text
							text = r.recognize_google(
								audio_data, language=selectedLanguage)
				...
				st.error('An error occured ...did you say anything?😵‍💫')
